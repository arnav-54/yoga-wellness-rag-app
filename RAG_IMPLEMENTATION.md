...
5. **RAG Service** (`services/ragService.js`)
   ...
   - Generates responses using OpenAI GPT-3.5-turbo-instruct
...
Construct Prompt with Context + Safety Instructions
    ↓
Generate Answer (GPT-3.5-turbo-instruct)
    ↓
...
2. **OpenAI API Errors**: Graceful fallback message to user
...
- OpenAI API dependency (cost and latency)
